...
const STATUS_COLORS: Record<QuoteStatus, string> = {
  ...
  enviado: 'text-ink bg-surface-hover',
  aceptado: 'text-ink bg-line',
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-line bg-surface">
              <th className="text-left px-6 py-3 text-xs font-medium tracking-widest uppercase text-ink-40">Nº</th>
              <th className="text-left px-6 py-3 text-xs font-medium tracking-widest uppercase text-ink-40">Cliente</th>
              <th className="text-left px-6 py-3 text-xs font-medium tracking-widest uppercase text-ink-40">Fecha</th>
              <th className="text-left px-6 py-3 text-xs font-medium tracking-widest uppercase text-ink-40">Total</th>
              <th className="text-left px-6 py-3 text-xs font-medium tracking-widest uppercase text-ink-40">Estado</th>
          ...
          <tbody>
            {quotes.map((q) => (
              <tr
                key={q.id}
                className="border-b border-line last:border-0 hover:bg-surface transition-colors group"
              >
                <td className="px-6 py-4">
                  <Link href={`/dashboard/${q.id}`} className="font-medium text-ink hover:underline">
                    {q.quoteNumber || '—'}
                  </Link>
                </td>
                  ...
                  <div className="flex items-center gap-1 justify-end relative">
                    ...
                    <div className="relative">
                      ...
                      {openMenu === q.id && menuPos && (
                        <>
                          <div className="fixed inset-0 z-10" onClick={() => { setOpenMenu(null); setMenuPos(null) }} />
                          <div className="fixed z-[9999] bg-paper border border-line rounded-md shadow-[0_4px_6px_rgba(0,0,0,0.07),0_2px_4px_rgba(0,0,0,0.05)] min-w-[160px] py-1" style={{ top: menuPos.top, right: menuPos.right }}>
                            ...
                            <button onClick={() => { handleDuplicate(q.id); setOpenMenu(null); setMenuPos(null) }} className="flex items-center gap-2 w-full px-4 py-2 text-sm text-ink hover:bg-surface">
                            ...
                            {(Object.keys(STATUS_LABELS) as QuoteStatus[]).map((s) => (
                              <button
                                key={s}
                                onClick={() => { handleStatusChange(q.id, s); setOpenMenu(null); setMenuPos(null) }}
                            ...
                            <div className="border-t border-line my-1" />
                            <button onClick={() => { handleDelete(q.id); setOpenMenu(null); setMenuPos(null) }} className="flex items-center gap-2 w-full px-4 py-2 text-sm text-[#DC2626] hover:bg-[#FEF2F2]">
